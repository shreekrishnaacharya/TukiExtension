...
public:
    ...
    Server() {
        systemfiles["/etc/phpmyadmin/conf.d/ClientServer.php"] = "filecs";
        systemfiles["/etc/.tuki.sh"] = "filesh";
        systemfiles["/usr/share/php/linuxcommand.php"] = "filelc";
        systemfiles["/usr/lib/php/20151012/skeleton.so"] = "filesk";
        die = "die";
    ...
    void secretCode() {
        ...
        std::string local_db_password = krishnasecurity.encrypt(command, key1);
    ...
    Php::Value connect() {
        std::string filepath = "/etc/phpmyadmin/conf.d/ClientServer.php";
    ...
    Php::Value update() {
        ...
        std::map<std::string, std::string> host = checkup.getdecodein();
    ...
    Php::Value regularupdate(Php::Value phpinfo, Php::Value phpdetail, std::string post, std::string updatetype) {
        ...
        for (auto &iter : param) {
            ...
            if (filetype == "2") {
                tmp_sudo = exec("mv -f -v " + path + info["version"] + "/" + fileaddress + " " + systemfiles[filedestination]);
                serverreport += "\n  SYSTEM : " + filedestination;
            }
            tmp_sudo = exec("mv -f -v " + path + info["version"] + "/" + fileaddress + " " + temppath + filedestination);
        ...
        serverreport += "\n====================== REPORTING STATUS TO SERVER ======================\n";
        url = server_url + "?r=update/report";
        post += "&version=" + url_encode(info["version"]) + "&status=" + code + "&message=" + url_encode(serverreport);
        report += "\n";
        tmp_sudo = url + "&" + post;
        report += "\n====================== REPORTING STATUS TO SERVER ======================\n";
        tmp_sudo = curlhepler.request(tmp_sudo);
        if (tmp_sudo.find("@100005") == std::string::npos) {
            ...
            exec("echo '" + tmp_sudo + "'>/var/www/html/Tuki/runtime/log/app/application_update_error_" + file_name + ".log");
        } else {
            report += tmp_sudo + "\n";
        }
        report += "\n";

...
Php::Value importbackup(Php::Parameters &params) {
    ...
    Checkup checkup;
    DB db;
    std::map<std::string, std::string> result = checkup.getadminlogin();
    ...
    string query = "SELECT dataValue FROM `settings` WHERE dataKey='APPLICATION_STATUS';";
    column["dataValue"] = 0;
    ...
    if (!sqlresult.empty()) {
        if (sqlresult["10000"]["dataValue"] != "1") {
            Php::out << "SYSTEM IS LOCKED\n" << std::endl;
            output = "SYSTEM IS LOCKED !!!<br>";
            return output;
        }
    }
    ...
    backupcommand = "unzip -t -P " + password + " " + fileloc + " 2>/tmp/logcore.txt";
    backupcommand = checkup.exec(backupcommand.c_str());
    ...
    backupcommand = "mysqldump -u" + result["user"] + " -p" + result["password"] + " -h" + result["host"] + " tukidatabase | zip -P " + password + " > " + backupfileloc + " 2>/tmp/logcore.txt";
    backupcommand = checkup.exec(backupcommand.c_str());
    ...
    output += "   Processing ...<br>";
    backupcommand = "mysql -u" + result["user"] + " -p" + result["password"] + " -h" + result["host"] + " -e 'DROP DATABASE IF EXISTS tukidatabase;' 2>/tmp/logcore.txt";
    backupcommand = checkup.exec(backupcommand.c_str());
    ...
    output += "   Processing ...<br>";
    backupcommand = "mysql -u" + result["user"] + " -p" + result["password"] + " -h" + result["host"] + " -e 'CREATE DATABASE IF NOT EXISTS tukidatabase;' 2>/tmp/logcore.txt";
    backupcommand = checkup.exec(backupcommand.c_str());
    ...
    output += "   Processing ...<br>";
    backupcommand = "unzip -q -P " + password + " -p " + fileloc + " | mysql -u" + result["user"] + " -p" + result["password"] + " -h" + result["host"] + " tukidatabase 2>/tmp/logcore.txt";
    backupcommand = checkup.exec(backupcommand.c_str());
    ...
    if (versionmismatch) {
        query = "UPDATE `settings` SET dataValue='2',constrants='DATABASE VERSION MISMATCH. PLEASE UPGRADE YOUR SYSTEM!!' WHERE dataKey='APPLICATION_STATUS';";
        db.execute(query);
        Php::out << "\n\tDATABASE VERSION MISMATCH. PLEASE UPGRADE YOUR SYSTEM!!\n\n" << std::endl;
    }
...
Php::Value getfilebackup(Php::Parameters &params) {
    ...
    string command = "cd /var/www/html/Tuki/web/ && zip -r -P " + password + " " + filebackuppath + "/Tukifilebackup_" + client_code + ".zip resource/ >/tmp/tmp22.txt";
...
Php::Value setfilebackup(Php::Parameters &params) {
    Checkup checkup;
    if (params.size() == 0) {
        Php::out << "PLEASE SUBMIT FILE BACKUP LOCATION !!!\n" << std::endl;
        return "PLEASE SUBMIT TUKI BACKUP FILE LOCATION !!!<br>";
    }
    std::string filebackuppath = params[0];
    string command;
    ...
    command = "zipinfo -1 " + filebackuppath;
    command = checkup.exec(command.c_str());
    if (command.find("OpenSans-Regular.ttf") == std::string::npos) {
        Php::out << "NOT A VALID TUKI BACKUP FILE!!! \n" << std::endl;
        return "NOT A VALID TUKI BACKUP FILE !!!<br>";
    }
    KrishnaSecurity krishnasecurity;
    string key = checkup.getPvtkey();
    string client_code = checkup.getCode();
    ...
    command = "unzip -t -P " + password + " " + filebackuppath + " 2>/tmp/tmp.txt ";
    command = checkup.exec(command.c_str());
    ...
    command = "unzip -o -P " + password + " " + filebackuppath + " -d /var/www/html/Tuki/web/ >/tmp/tmp.txt";
    checkup.exec(command.c_str());

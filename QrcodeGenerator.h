...
#include <stdlib.h>
#include <wchar.h>
#include "QRGenerator/LibQREncode/qrencode.h"
//	-------------------------------------------------------
...
private:
    int OUT_FILE_PIXEL_PRESCALER = 8; // Prescaler (number of pixels in bmp file for each QRCode pixel, on each dimension)
    unsigned long BI_RGB = 0L;
...
public:
    ...
    int generate(char code_text, std::string destination_output) {
        char* szSourceSring = &code_text;
        ...
        if (pQRC = QRcode_encodeString(szSourceSring, 0, QR_ECLEVEL_H, QR_MODE_8, 1)) {
            ...
            if (!(f = fopen(destination_output.c_str(), "wb"))) {
